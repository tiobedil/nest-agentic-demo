...
import { ChevronDown, Check, Sparkle } from "lucide-react"
  ...
  return (
    <div className="w-full">
      <button onClick={() => setOpen(v => !v)} className="flex w-fit items-center gap-2 text-left pl-[1px]">
        ...
        <span className={working ? "bg-clip-text text-sm font-medium text-transparent" : "text-sm font-medium text-muted-foreground"} style={working ? { backgroundImage: "linear-gradient(90deg, var(--muted-foreground) 35%, var(--foreground) 50%, var(--muted-foreground) 65%)", backgroundSize: "200% 100%", animation: "shimmer-text 1.4s linear infinite" } : undefined}>{working ? "Thinking" : "Thought for 6 seconds"}</span>
        ...
        <div className="relative mt-2 ml-[7px] border-l border-border/20 pl-5 flex flex-col gap-2 py-1">
          {rows.slice(0, forcedDone ? rows.length : Math.min(stage + 1, rows.length)).map((r, i) => (
            <div key={r} className="flex items-center gap-2 text-xs leading-none text-muted-foreground/70">
              <span className="flex size-3 shrink-0 items-center justify-center">{(forcedDone || i < stage) ? <Check className="size-3 text-muted-foreground/60" /> : <span className="size-1.5 rounded-full bg-muted-foreground/30" />}</span>
              {r}
            </div>
          ))}
        </div>
