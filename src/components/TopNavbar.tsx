...
const NAV_ITEMS: NavItem[] = [
  ...
  { label: "AI Agent", to: "__ai-agent-dropdown__", dropdown: true },
  { label: "Inbox", to: "#" },
...
function ChevronDown({ open }: { open?: boolean }) {
  return (
    <svg
      ...
      className={`ml-1 shrink-0 opacity-60 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
    >
      <path d="m6 9 6 6 6-6" />
    </svg>
  )
...
function AIAgentDropdown() {
  const [open, setOpen] = useState(false)
  const ref = useRef<HTMLDivElement>(null)
  const location = useLocation()
  const isActive = AI_AGENT_ITEMS.some(i => (i.to === "/" ? location.pathname === "/" : location.pathname === i.to))
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen(v => !v)}
        className={`flex items-center rounded-md px-3 py-[6px] text-[14px] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 ${
          isActive
      ...
      {open && (
        <div
          role="menu"
          className="absolute left-0 top-full z-50 mt-2 min-w-[220px] rounded-xl bg-white p-1.5 shadow-lg animate-in fade-in-0 zoom-in-95"
          style={{ boxShadow: "0 8px 24px rgba(15,23,42,0.08), 0 2px 8px rgba(15,23,42,0.06)" }}
        >
          <div className="flex flex-col gap-1">
            {AI_AGENT_ITEMS.map(item => (
              <NavLink
                key={item.to}
                to={item.to}
                end={item.to === "/"}
                ...
                className={({ isActive: itemActive }) =>
                  `flex items-center rounded-lg px-2.5 py-2.5 text-[14px] leading-none transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 ${
                    itemActive ? "bg-primary/10 font-medium text-primary" : "font-normal text-slate-700 hover:bg-muted hover:text-foreground"
                  }`
                }
              >
    ...
    <header className="sticky top-0 z-50 flex h-[60px] items-center justify-between border-b border-[#2E303A26] bg-background px-8">
      <div className="flex items-center">
        ...
        <nav className="ml-[24px] hidden items-center gap-[4px] lg:flex">
          {NAV_ITEMS.map(item => {
            if (item.label === "AI Agent") {
              return <AIAgentDropdown key={item.label} />
            }
            if (item.to === "#") {
              return (
                <span
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center rounded-md px-3 py-[6px] text-[14px] transition-colors ${
                    isActive ? "bg-primary/10 font-semibold text-primary" : "text-slate-800 hover:bg-gray-100 hover:text-slate-800"
                  }`
                }
              >
                {item.label}
